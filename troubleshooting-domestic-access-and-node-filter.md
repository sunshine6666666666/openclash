...
## 9.1 机场订阅时效与网络前提（客服确认）

客服确认要点（按原意整理）：
- 订阅链接可能在短时间内失效（常见约 1 小时）。
- 下载订阅时不要使用 VPN，应使用国内网络直连。
- 云服务商网络可能触发 `403`，并非一定是链接本身错误。
- 订阅条目过多会触发 `401`（示例：超过 5 个），需精简订阅或重置后再试。
- 若仍失败，优先切换网络环境（如手机 4G/5G 热点）再更新。

执行建议（避免再次误判）：
- 先确认“当前链接是否刚复制且未过期”，再点击更新。
- 一次只更新 1 条订阅，避免并发更新导致状态混乱。
- 更新失败先看状态码：`403` 多为网络/区域策略，`401` 多为鉴权/配额策略。
...
### 11.1 什么时候“立刻收工”
- 当且仅当出现 `国内OK + 国外OK`，立刻停止改动。
- 此时继续“追求更优延迟”往往带来回退风险，不值得。

### 11.2 关于“全局 vs 规则”的实战结论
- 理论上规则模式更优雅，但前提是订阅策略组设计完善（有自动故障转移）。
- 当前机场配置偏手动组，实操中“全局 + 稳定节点”更可用。
- 因此本次采用：先保可用，再等待机场提供更完善策略组后再升级规则模式。

### 11.3 订阅窗口策略（5 分钟）
- 新链接有短时效时，不做多轮命令实验。
- 复制最新链接后，优先完成“保存 + 更新 + 验证”闭环。
- 超时失败立即放弃本轮，等待下一个新链接，避免重复消耗时间窗口。

### 11.4 以后默认动作
- 每次上线先跑“一分钟应急口令”。
- 结果双 OK：直接使用，不改配置。
- 出现问题再按“网络层 -> DNS -> 订阅/鉴权 -> 节点”顺序处理。
